...
def left():
    global move
    if move == [1,0]: return [0, -1]
    elif move == [0, -1]: return [-1, 0]
    elif move == [-1, 0]: return [0, 1]
    elif move == [0, -1]: return [1, 0]
...
def left_cell(pos):
    l = left()
    return [pos[i] + l[i] for i in [0, 1]]

def forward_cell(pos):
    return [pos[i] + move[i] for i in [0, 1]]

def hero_strategy(agent):
    global move
    if agent.pause > 0:
        return (0, 0)
    if not agent.game.visitable(left_cell(agent.pos)):
        if agent.game.visitable(forward_cell(agent.pos)):
            return move
        else:
            move = right()
            return (0, 0)
