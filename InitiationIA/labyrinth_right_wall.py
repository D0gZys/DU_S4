...
def Zhero_strategy(agent):
    moves = [(0, 1), (1, 0), (0, -1), (-1, 0)]
    left_Wall = [(0, 1), (1, 0), (0, -1), (-1, 0)]
    for dx, dy in left_Wall:
...
def hero_strategy(agent):
    global last_pos
    if (agent.game.visitable((agent.pos[0], agent.pos[1] + 1)) and last_pos != (0, -1)) or (
            not agent.game.visitable((agent.pos[0], agent.pos[1] - 1)) and not agent.game.visitable(
            (agent.pos[0] - 1, agent.pos[1])) and not agent.game.visitable((agent.pos[0] + 1, agent.pos[1]))):
        last_pos = (0, 1)
        return (0, 1)

    elif (agent.game.visitable((agent.pos[0] + 1, agent.pos[1])) and last_pos != (-1, 0)) or (
            not agent.game.visitable((agent.pos[0] - 1, agent.pos[1])) and not agent.game.visitable(
            (agent.pos[0], agent.pos[1] - 1)) and not agent.game.visitable((agent.pos[0], agent.pos[1] + 1))):
        last_pos = (1, 0)
        return (1, 0)

    elif (agent.game.visitable((agent.pos[0], agent.pos[1] - 1)) and last_pos != (0, 1)) or (
            not agent.game.visitable((agent.pos[0] + 1, agent.pos[1])) and not agent.game.visitable(
            (agent.pos[0], agent.pos[1] + 1)) and not agent.game.visitable((agent.pos[0] - 1, agent.pos[1]))):
        last_pos = (0, -1)
        return (0, -1)
...
class Game:
    ...
    def __init__(self, size=maze_size, sparsness=None):
        ...
        for i in range(enemy_nb):
            self.enemies.append(
                Enemy(self, chr(ord('A') + i), self.pick_free_pos(close_to=(size[0] // 2, size[1] // 2))))
